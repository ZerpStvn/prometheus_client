...
const ProjectForm = () => {
  ...
  const [grossSquareFootage, setGrossSquareFootage] = useState<number | string>(
    "",
  );
  ...
  const convertToSquareMeters = (sqft: number) => {
    return (sqft * 0.092903).toFixed(2); // Conversion factor for square feet to square meters
  };

  const convertedValue =
    unit === "sqm"
      ? grossSquareFootage && !isNaN(Number(grossSquareFootage))
        ? convertToSquareMeters(Number(grossSquareFootage))
        : ""
      : grossSquareFootage;
